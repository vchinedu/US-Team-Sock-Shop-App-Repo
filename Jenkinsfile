pipeline {
  ...
  stages {
    stage ('deploying to stage environment'){
      steps {
        sshagent(['ansible-key']) {
          sh 'ssh -t -t ubuntu@10.0.2.45 -o StrictHostKeyChecking=no "ansible-playbook /home/ubuntu/playbook/stage.yml"'
        }
      }
    }
    ...
    stage ('deploying to prod environment'){
      steps {
        sshagent(['ansible-key']) {
          sh 'ssh -t -t ubuntu@10.0.2.45 -o StrictHostKeyChecking=no "ansible-playbook /home/ubuntu/playbook/prod.yml"'
        }
      }
    }
  }
